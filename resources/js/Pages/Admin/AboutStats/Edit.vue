...
const form = useForm({
    value: props.aboutStat.value || '',
    label: props.aboutStat.label || '',
    display_order: props.aboutStat.display_order || 0,
    is_active: props.aboutStat.is_active ?? true,
});

const submit = () => {
    form.put(route('admin.about-stats.update', props.aboutStat.id));
};
</script>

<template>
    <Head title="Edit About Stat" />
